...
contract Lottery is OwnableUpgradeable {
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external {
        ...
        elxr.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function multiBuy(uint256 _price, uint8[4][] memory _numbers) external {
        ...
        elxr.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if(reward>0) {
            safeElxrTransfer(address(msg.sender), reward);
        }
    ...
    function multiClaim(uint256[] memory _tickets) external {
        ...
        if(totalReward>0) {
            safeElxrTransfer(address(msg.sender), totalReward);
        }
    ...
    function safeElxrTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > elxrBal) {
            elxr.transfer(_to, elxrBal);
        } else {
            elxr.transfer(_to, _amount);
        }
    }
    ...
    function adminWithdraw(uint256 _amount) external onlyAdmin {
        elxr.safeTransfer(address(msg.sender), _amount);
